...
func main() {
	...
	searchProvider := flag.String("search-provider", "", "Search provider: exa, google, serpingapi or serply (Env: SEARCH_PROVIDER)")
	...
	if len(os.Args) > 1 {
		...
		case *isFind:
			...
			if len(prompt) > 0 {
				...
				supportedSearchProviders := map[string]bool{"google": true, "exa": true, "serpingapi": true, "serply": true, "": true}
