...
const (
	...
	defaultURL   = "https://api.orcarouter.ai/v1/chat/completions"
)
...
func model(params structs.Params) string {
	...
	if value := os.Getenv("ORCAROUTER_MODEL"); value != "" {
		return value
	}
	return defaultModel
}
...
func apiKey(params structs.Params) string {
	...
	if value := os.Getenv("ORCAROUTER_API_KEY"); value != "" {
		return value
	}
...
func endpoint(params structs.Params) string {
	...
	if value := os.Getenv("ORCAROUTER_URL"); value != "" {
		return value
	}
	if value := os.Getenv("ORCAROUTER_BASE_URL"); value != "" {
...
func NewRequest(input string, params structs.Params) (*http.Response, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create OrcaRouter request: %w", err)
	}
