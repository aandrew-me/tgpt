...
const (
	...
	defaultURL   = "https://api.cheaperinference.com/v1/chat/completions"
)
...
func model(params structs.Params) string {
	...
	if value := os.Getenv("CHEAPERINFERENCE_MODEL"); value != "" {
		return value
	}
	return defaultModel
}
...
func apiKey(params structs.Params) string {
	...
	if value := os.Getenv("CHEAPERINFERENCE_API_KEY"); value != "" {
		return value
	}
...
func endpoint(params structs.Params) string {
	...
	if value := os.Getenv("CHEAPERINFERENCE_URL"); value != "" {
		return value
	}
	if value := os.Getenv("CHEAPERINFERENCE_BASE_URL"); value != "" {
...
func NewRequest(input string, params structs.Params) (*http.Response, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("create Cheaper Inference request: %w", err)
	}
